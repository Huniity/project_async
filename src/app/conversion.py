...
def convert_csv_to_json(csv_file, json_file, chunk_size=10000):
    ...
    logger.info(f"Starting conversion: {os.path.basename(csv_file)} ({file_size / 1024 / 1024:.2f}MB)")
    ...
    try:
        with open(json_file, 'w') as out_f:
            for chunk in pd.read_csv(csv_file, chunksize=chunk_size):
                ...
                if chunk_count % 50 == 0:
                    logger.debug(f"  → Processed {chunk_count} chunks ({total_rows:,} rows)")
        
        elapsed = time.time() - start_time
        output_size = os.path.getsize(json_file)
        logger.info(
            f"✓ Completed: {os.path.basename(csv_file)} → {os.path.basename(json_file)} | "
            f"Chunks: {chunk_count} | Rows: {total_rows:,} | "
            f"Output: {output_size / 1024 / 1024:.2f}MB | Time: {elapsed:.2f}s"
        )
        return elapsed
...
def run(input_dir: str = "input", output_dir: str = "output"):
    start_all = time.time()
    logger.info("=" * 70)
